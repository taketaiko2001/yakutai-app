...
(function (global) {
  ...
  function defaults() {
    return {
      ...
      sites: clone(D.sites),
      rules: clone(D.rules),
      learn: { presets: {}, usage: {}, gaiyou: {}, ocrFix: {}, count: 0 },
    };
  }
  let data;
  function load() {
  ...
  function ruleMatches(rule, bag) {
    ...
    switch (rule.kind) {
      ...
      case "containers_at_least": return (+bag.containers || 0) >= rule.n;
  ...
  function ruleText(r) {
    ...
    switch (r.kind) {
      ...
      case "containers_at_least": return `混合容器が${r.n}個以上 → ${big}`;
  ...
  // 印刷を確定したときに呼ぶ。bags: 確定した袋、ocrInitial: 読み取り直後の「カルテの内容」、finalText: 確定時の内容
  function learnFrom(bags, ocrInitial, finalText) {
    ...
    // 読み取り直後の「#」行（読めなかった行）が、同じ位置で人の入力に置き換わっていたら覚える
    if (ocrInitial) {
      const a = ocrInitial.split("\n"), f = finalText.split("\n");
      for (const [i, j] of alignReplaced(a, f)) {
        const from = a[i].replace(/^#\s*/, "").trim(), to = f[j].trim();
        const k = global.KarteParser.fuzzkey(from);
  ...
  function alignReplaced(a, f) {
    ...
    while (i < n && j < m) {
      ...
      if (dp[i + 1][j] === dp[i][j + 1] && a[i].trim().startsWith("#") && !f[j].trim().startsWith("#")) { pairs.push([i, j]); i++; j++; continue; }
