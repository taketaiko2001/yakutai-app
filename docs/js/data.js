// 初期データ（アプリ内で編集した内容は端末に保存され、こちらより優先されます）
window.DEFAULT_DATA = {
 "version": 11,
 "drugs": [
  {
   "name": "シナール配合錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "しなーる",
    "シナール"
   ],
   "note": "",
   "common": true,
   "dose": "3T",
   "adopted": true
  },
  {
   "name": "トラネキサム酸錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "とらねきさむ",
    "トラネキサム",
    "トランサミン"
   ],
   "note": "",
   "common": true,
   "dose": "3T",
   "adopted": true
  },
  {
   "name": "ユベラ錠50mg",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "ゆべら",
    "ユベラ"
   ],
   "note": "",
   "common": true,
   "dose": "3T",
   "adopted": true
  },
  {
   "name": "レボセチリジン塩酸塩錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "れぼせちりじん",
    "レボセチリジン",
    "ザイザル"
   ],
   "note": "",
   "common": true,
   "dose": "1T",
   "adopted": true
  },
  {
   "name": "ロラタジン錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "ロラタジン",
    "クラリチン",
    "ろらたじん"
   ],
   "note": "",
   "common": true,
   "dose": "1T",
   "adopted": true
  },
  {
   "name": "フェキソフェナジン塩酸塩錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "ふぇきそふぇなじん",
    "アレグラ"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "オロパタジン塩酸塩錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "おろぱたじん",
    "アレロック",
    "オロパ",
    "オロパタジン"
   ],
   "note": "",
   "common": true,
   "dose": "2T",
   "adopted": true
  },
  {
   "name": "エピナスチン塩酸塩錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "えぴなすちん",
    "アレジオン"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ビラノア錠",
   "fixedUsage": {"times": "1", "timing": ["ねる前"], "meal": ""},
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "びらのあ",
    "ビラスチン",
    "ビラノア"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "デザレックス錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "でざれっくす",
    "デスロラタジン"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ルパフィン錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "るぱふぃん"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ベポタスチンベシル酸塩錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "べぽたすちん",
    "タリオン"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "クラリスロマイシン錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "くらりす",
    "クラリス"
   ],
   "note": ""
  },
  {
   "name": "ミノサイクリン塩酸塩錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "みのまいしん",
    "ミノマイシン",
    "ミノ"
   ],
   "note": "",
   "common": true,
   "adopted": true
  },
  {
   "name": "ドキシサイクリン塩酸塩錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "びぶらまいしん",
    "ビブラマイシン",
    "ドキシ"
   ],
   "note": ""
  },
  {
   "name": "ロキシスロマイシン錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "るりっど",
    "ルリッド"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ファロム錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "ふぁろむ"
   ],
   "note": ""
  },
  {
   "name": "フロモックス錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "セフカペンピボキシル塩酸塩錠",
    "ふろもっくす",
    "フロモックス",
    "セフカペン"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "バラシクロビル錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "ばるとれっくす",
    "バルトレックス",
    "バラシクロビル"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ファムシクロビル錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "ふぁむびる",
    "ファムビル"
   ],
   "note": ""
  },
  {
   "name": "アメナリーフ錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "あめなりーふ"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "テルビナフィン錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "てるびなふぃん",
    "ラミシール錠"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "イトラコナゾールカプセル",
   "type": "naifuku",
   "form": "カプセル",
   "times": "",
   "aliases": [
    "いとりぞーる",
    "イトリゾール",
    "イトラコナゾール"
   ],
   "note": ""
  },
  {
   "name": "プレドニン錠5mg",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "プレドニゾロン錠",
    "ぷれどにん",
    "プレドニン",
    "PSL",
    "プレドニゾロン"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ロキソプロフェンNa錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "ろきそにん",
    "ロキソニン"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "カロナール錠200",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "アセトアミノフェン錠",
    "かろなーる",
    "カロナール",
    "アセトアミノフェン"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "レバミピド錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "むこすた",
    "ムコスタ"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "L-システイン錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "はいちおーる",
    "ハイチオール"
   ],
   "note": ""
  },
  {
   "name": "十味敗毒湯エキス顆粒",
   "fixedUsage": {"times": "2", "timing": ["朝", "夕"], "meal": "食前"},
   "type": "naifuku",
   "form": "こな薬",
   "times": "",
   "aliases": [
    "じゅうみはいどくとう",
    "十味",
    "十味敗毒湯",
    "ジュウミハイドクトウ"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "荊芥連翹湯エキス顆粒",
   "type": "naifuku",
   "form": "こな薬",
   "times": "",
   "aliases": [
    "けいがいれんぎょうとう"
   ],
   "note": ""
  },
  {
   "name": "抑肝散エキス顆粒",
   "type": "naifuku",
   "form": "こな薬",
   "times": "",
   "aliases": [
    "よくかんさん"
   ],
   "note": ""
  },
  {
   "name": "ヘパリン類似物質ローション",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "ヘパlo",
    "ヘパローション",
    "へぱろーしょん",
    "ヒルドイドローション",
    "ヘパロ",
    "ヒルlo",
    "ヒルドイドlo",
    "ヘパlo乳"
   ],
   "note": "",
   "common": true,
   "adopted": true,
   "site": "顔保湿"
  },
  {
   "name": "ヘパリン類似物質油性クリーム",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "ヘパcr",
    "ヘパクリーム",
    "ヒルドイドクリーム",
    "ヘパcn",
    "ヘパリン類似物質クリーム",
    "ヘパ油性",
    "ヘパソフト",
    "ヒルドイドソフト",
    "ヒルドイドソフト軟膏"
   ],
   "note": "",
   "adopted": true,
   "common": true,
   "site": "顔保湿"
  },
  {
   "name": "ヘパリン類似物質スプレー",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "ヘパsp",
    "ヘパスプレー",
    "ヘパ泡",
    "ヘパリン泡"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "パンデルローション",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "パンデルlo"
   ],
   "note": "",
   "common": true,
   "adopted": true,
   "site": "頭"
  },
  {
   "name": "パンデル軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "パンデルo",
    "パンデル軟"
   ],
   "note": ""
  },
  {
   "name": "パンデルクリーム",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "パンデルcr"
   ],
   "note": ""
  },
  {
   "name": "ゲンタマイシン硫酸塩軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "ゲンタシン軟膏",
    "GMo",
    "GM軟膏",
    "GM",
    "げんたしん",
    "ゲンタマイシン",
    "ゲンタシン",
    "ゲンタマイシンo"
   ],
   "note": "",
   "common": true,
   "adopted": true,
   "site": "キズ"
  },
  {
   "name": "リンデロンVG軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "VGo",
    "リンデロンVG",
    "りんでろん"
   ],
   "note": ""
  },
  {
   "name": "ロコイド軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "ロコイドo",
    "ろこいど",
    "ロコイド"
   ],
   "note": "",
   "adopted": true,
   "common": true,
   "site": "顔わるいところ"
  },
  {
   "name": "クロベタゾン酪酸エステル軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "クロらくo",
    "クロらく",
    "クロラクo",
    "クロ酪o",
    "クロ酪",
    "くろらく",
    "キンダベート軟膏",
    "きんだべーと",
    "キンダベート"
   ],
   "note": "キンダベートの後発品",
   "common": true,
   "adopted": true
  },
  {
   "name": "アンテベート軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "あんてべーと",
    "アンテベートo",
    "アンテベート"
   ],
   "note": "",
   "adopted": true,
   "common": true
  },
  {
   "name": "マイザー軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "まいざー"
   ],
   "note": ""
  },
  {
   "name": "クロベタゾールプロピオン酸エステル軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "デルモベート軟膏",
    "でるもべーと",
    "デルモベート",
    "デルモo",
    "クロ(P)o",
    "クロPo",
    "クロベタゾール軟膏"
   ],
   "note": "",
   "adopted": true,
   "common": true
  },
  {
   "name": "プロトピック軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "ぷろとぴっく"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "コレクチム軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "これくちむ",
    "コレクチムo",
    "コレクチム"
   ],
   "note": "",
   "adopted": true,
   "common": true
  },
  {
   "name": "モイゼルト軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "もいぜると"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ブイタマークリーム",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "1",
   "aliases": [
    "ぶいたまー",
    "ブイタマーcr",
    "ブイタマーcn",
    "ブイタマー"
   ],
   "note": "添付文書の用法: 1日1回",
   "adopted": true,
   "common": true,
   "site": "顔"
  },
  {
   "name": "白色ワセリン",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "わせりん",
    "ワセリン",
    "プロペト",
    "白ワセ",
    "白色ワセ",
    "ハクワセ",
    "白ワセリン"
   ],
   "note": "容器に入れるときは「白ワセ-1」のように容器番号を書く。口に塗るときは1日数回",
   "adopted": true,
   "common": true
  },
  {
   "name": "亜鉛華軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "あえんか"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ウレパールクリーム",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "うれぱーる"
   ],
   "note": ""
  },
  {
   "name": "アズノール軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "あずのーる"
   ],
   "note": ""
  },
  {
   "name": "ルリコナゾールクリーム",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "1",
   "aliases": [
    "ルリコンクリーム",
    "るりこん",
    "ルリコン",
    "ルリコナゾール",
    "ルリコンcn",
    "るりこなぞーる"
   ],
   "note": "添付文書の用法: 1日1回",
   "adopted": true,
   "site": "足"
  },
  {
   "name": "テルビナフィン塩酸塩クリーム",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "1",
   "aliases": [
    "ラミシールクリーム",
    "らみしーる",
    "ラミシール",
    "テルビナフィンcn",
    "テルビナフィンクリーム"
   ],
   "note": "添付文書の用法: 1日1回",
   "adopted": true,
   "site": "足"
  },
  {
   "name": "アスタットクリーム",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "1",
   "aliases": [
    "あすたっと"
   ],
   "note": "添付文書の用法: 1日1回"
  },
  {
   "name": "ゼビアックスローション",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "1",
   "aliases": [
    "ぜびあっくす",
    "ゼビアックスlo"
   ],
   "note": "添付文書の用法: 1日1回",
   "common": true,
   "site": "顔ニキビ"
  },
  {
   "name": "ナジフロキサシンクリーム",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "アクアチムクリーム",
    "あくあちむ",
    "アクアチム",
    "ナジフロキサシン",
    "なじふろきさしん",
    "ナジフロキサシンcr",
    "ナジフロキサシンcn"
   ],
   "note": "",
   "adopted": true,
   "common": true,
   "site": "顔ニキビ"
  },
  {
   "name": "ダラシンTローション",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "ダーTlo",
    "ダーT",
    "ダラシンTlo",
    "ダラシンT",
    "だらしんてぃー",
    "ダTlo",
    "ダT"
   ],
   "note": "",
   "common": true,
   "adopted": true,
   "site": "頭"
  },
  {
   "name": "ディフェリンゲル",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "1",
   "aliases": [
    "でぃふぇりん"
   ],
   "note": "添付文書の用法: 1日1回 就寝前",
   "adopted": true,
   "site": "顔"
  },
  {
   "name": "ベピオゲル",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "1",
   "aliases": [
    "べぴお"
   ],
   "note": "添付文書の用法: 1日1回",
   "adopted": true,
   "site": "顔"
  },
  {
   "name": "エピデュオゲル",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "1",
   "aliases": [
    "えぴでゅお"
   ],
   "note": "添付文書の用法: 1日1回"
  },
  {
   "name": "デュアック配合ゲル",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "1",
   "aliases": [
    "でゅあっく"
   ],
   "note": "添付文書の用法: 1日1回"
  },
  {
   "name": "ロゼックスゲル",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "ろぜっくす"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ドボベット軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "1",
   "aliases": [
    "どぼべっと"
   ],
   "note": "添付文書の用法: 1日1回",
   "adopted": true
  },
  {
   "name": "マーデュオックス軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "1",
   "aliases": [
    "まーでゅおっくす"
   ],
   "note": "添付文書の用法: 1日1回"
  },
  {
   "name": "オキサロール軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "おきさろーる"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "クレナフィン爪外用液",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "夜1",
   "aliases": [
    "くれなふぃん",
    "クレナフィン",
    "クレナフィンlo"
   ],
   "note": "いつも1日夜1回（カルテに回数がなくても）",
   "adopted": true,
   "common": true,
   "fixedTimes": true,
   "site": "爪"
  },
  {
   "name": "ルコナック爪外用液",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "夜1",
   "aliases": [
    "るこなっく"
   ],
   "note": "いつも1日夜1回（カルテに回数がなくても）",
   "adopted": true,
   "fixedTimes": true,
   "site": "爪"
  },
  {
   "name": "フシジンレオ軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "ふしじん"
   ],
   "note": ""
  },
  {
   "name": "オイラックスクリーム",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "おいらっくす"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "モーラステープ",
   "type": "gaiyou",
   "form": "貼り薬",
   "times": "1",
   "aliases": [
    "もーらす"
   ],
   "note": "添付文書の用法: 1日1回"
  },
  {
   "name": "サヘパ（混合軟膏）",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "サヘパ",
    "サヘパー",
    "さへぱ"
   ],
   "note": "サレックス＋ヘパリン類似物質の混合。容器番号 2番=30g・3番=50g・4番=100g（例: サヘパ-3×2）",
   "mix": true,
   "common": true,
   "adopted": true
  },
  {
   "name": "ベタヘパ（混合軟膏）",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "ベタヘパ",
    "ベヘパ",
    "べたへぱ"
   ],
   "note": "ベタ＋ヘパリン類似物質の混合。容器番号 2番=30g・3番=50g・4番=100g（例: ベタヘパ-3×2）",
   "mix": true,
   "common": true,
   "adopted": true
  },
  {
   "name": "ロヘパ（混合軟膏）",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "ロヘパ",
    "ろへぱ"
   ],
   "note": "ロコイド＋ヘパリン類似物質の混合。容器番号 2番=30g・3番=50g・4番=100g（例: ロヘパ-3×2）",
   "mix": true,
   "common": true,
   "adopted": true
  },
  {
   "name": "クロヘパ（混合軟膏）",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "クロヘパ",
    "くろへぱ"
   ],
   "note": "クロ＋ヘパリン類似物質の混合。容器番号 2番=30g・3番=50g・4番=100g（例: クロヘパ-3×2）",
   "mix": true,
   "common": true,
   "adopted": true
  },
  {
   "name": "サZ（混合軟膏）",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "サZ",
    "サz",
    "サゼット"
   ],
   "note": "サレックス＋亜鉛華軟膏の混合。容器番号 2番=30g・3番=50g・4番=100g（例: サZ-3×2）",
   "mix": true,
   "common": true,
   "adopted": true
  },
  {
   "name": "ベZ（混合軟膏）",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "ベZ",
    "ベz",
    "ベタZ",
    "ベゼット"
   ],
   "note": "ベタ＋亜鉛華軟膏の混合。容器番号 2番=30g・3番=50g・4番=100g（例: ベZ-3×2）",
   "mix": true,
   "common": true,
   "adopted": true
  },
  {
   "name": "ロZ（混合軟膏）",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "ロZ",
    "ロz",
    "ロゼット"
   ],
   "note": "ロコイド＋亜鉛華軟膏の混合。容器番号 2番=30g・3番=50g・4番=100g（例: ロZ-3×2）",
   "mix": true,
   "common": true,
   "adopted": true
  },
  {
   "name": "クロZ（混合軟膏）",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "クロZ",
    "クロz",
    "クロゼット"
   ],
   "note": "クロ＋亜鉛華軟膏の混合。容器番号 2番=30g・3番=50g・4番=100g（例: クロZ-3×2）",
   "mix": true,
   "common": true,
   "adopted": true
  },
  {
   "name": "ゼビアックス油性クリーム",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "1",
   "aliases": [
    "ゼビアックスcr",
    "ゼビアックスcn",
    "ゼビアックスクリーム",
    "ぜびあっくすくりーむ"
   ],
   "note": "添付文書の用法: 1日1回",
   "common": true,
   "adopted": true,
   "site": "顔ニキビ"
  },
  {
   "name": "ベピオウォッシュゲル",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "1",
   "aliases": [
    "ベピオウォッシュ",
    "べぴおうぉっしゅ",
    "ベピオwa",
    "ベピオワ",
    "ベピオwg",
    "ベピオWG",
    "ベピオウォッシュゲル"
   ],
   "note": "添付文書の用法: 1日1回",
   "common": true,
   "adopted": true,
   "site": "顔"
  },
  {
   "name": "クリンダマイシンゲル",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "クリンダゲル",
    "クリーゲル",
    "クリンダ",
    "ダラシンゲル",
    "くりんだ",
    "ダラシンTゲル"
   ],
   "note": "",
   "common": true,
   "adopted": true,
   "site": "顔ニキビ"
  },
  {
   "name": "デルモベートスカルプローション",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "デルモlo",
    "デルモ",
    "でるもろーしょん"
   ],
   "note": "",
   "common": true,
   "site": "頭"
  },
  {
   "name": "クロベタゾールプロピオン酸エステルローション",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "クロ(P)lo",
    "クロPlo",
    "クロlo",
    "くろべたぞーる"
   ],
   "note": "",
   "common": true,
   "adopted": true,
   "site": "頭"
  },
  {
   "name": "ケラチナミンクリーム",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "ケラチナミン",
    "ケラチナミンcn",
    "けらちなみん"
   ],
   "note": "",
   "common": true,
   "adopted": true,
   "site": "足かさかさ"
  },
  {
   "name": "塩化アルミニウム液",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "",
   "aliases": [
    "塩Al",
    "塩化Al",
    "塩アル",
    "えんかあるみにうむ"
   ],
   "note": "院内製剤（例: 塩Al 60cc 夜1）",
   "common": true,
   "adopted": true,
   "site": "足汗止め"
  },
  {
   "name": "レスタミンコーワクリーム",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "レスタミン",
    "レスタミンクリーム",
    "れすたみん"
   ],
   "note": "",
   "common": true,
   "adopted": true
  },
  {
   "name": "スピラゾン軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "スピラゾン",
    "スピラゾンo",
    "すぴらぞん"
   ],
   "note": "",
   "common": true,
   "adopted": true
  },
  {
   "name": "トプシムクリーム",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "トプシム",
    "トプシムcn",
    "とぷしむ",
    "トプシムcr"
   ],
   "note": "",
   "common": true,
   "adopted": true
  },
  {
   "name": "d-クロルフェニラミンマレイン酸塩錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "クロルフェニラミン",
    "ポララミン",
    "d-クロル"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "PL配合顆粒",
   "type": "naifuku",
   "form": "こな薬",
   "times": "",
   "aliases": [
    "PL顆粒",
    "PL配合"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "アドナ錠30mg",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "アドナ",
    "あどな"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "アレロック顆粒",
   "type": "naifuku",
   "form": "こな薬",
   "times": "",
   "aliases": [
    "アレロック顆粒",
    "オロパタジン顆粒"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "エバスチンOD錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "エバスチン",
    "エバステル",
    "えばすちん"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "エンペラシン配合錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "エンペラシン",
    "セレスタミン",
    "えんぺらしん"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "オテズラ錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "オテズラ",
    "おてずら"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ケトチフェンシロップ",
   "type": "naifuku",
   "form": "こな薬",
   "times": "",
   "aliases": [
    "ケトチフェン",
    "ザジテン",
    "ケトチフェンsy"
   ],
   "note": "シロップ。のみぐすり袋の「こなぐすり」の欄に「シロップ1本」と書く",
   "adopted": true,
   "syrup": true
  },
  {
   "name": "酢酸亜鉛錠50mg",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "酢酸亜鉛",
    "ノベルジン",
    "サクサンアエン"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "シナール配合顆粒",
   "type": "naifuku",
   "form": "こな薬",
   "times": "",
   "aliases": [
    "シナール顆粒",
    "シナールG"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "シプロキサン錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "シプロキサン",
    "シプロフロキサシン",
    "しぷろきさん"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ストロメクトール錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "ストロメクトール",
    "イベルメクチン",
    "すとろめくとーる"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "セファランチン錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "セファランチン",
    "せふぁらんちん"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "セフポドキシムプロキセチル錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "セフポドキシム",
    "バナン錠",
    "セフポ"
   ],
   "note": "",
   "adopted": true,
   "common": true
  },
  {
   "name": "ネイリンカプセル",
   "type": "naifuku",
   "form": "カプセル",
   "times": "",
   "aliases": [
    "ネイリン",
    "ねいりん"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "バルトレックス顆粒",
   "type": "naifuku",
   "form": "こな薬",
   "times": "",
   "aliases": [
    "バルトレックス顆粒"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ピリドキサール錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "ピリドキサール",
    "ピドキサール",
    "ぴりどきさーる"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ファロムドライシロップ小児用",
   "type": "naifuku",
   "form": "こな薬",
   "times": "",
   "aliases": [
    "ファロムDS",
    "ファロムドライシロップ"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "プレガバリンOD錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "プレガバリン",
    "リリカ",
    "ぷれがばりん"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "プロ・バンサイン錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "プロバンサイン",
    "ぷろばんさいん"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ミヤBM錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "ミヤBM",
    "ミヤ",
    "みやびーえむ"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "メイアクトMS小児用細粒",
   "type": "naifuku",
   "form": "こな薬",
   "times": "",
   "aliases": [
    "メイアクト",
    "めいあくと"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ヨクイニンエキス錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "ヨクイニン",
    "よくいにん"
   ],
   "note": "",
   "adopted": true,
   "common": true
  },
  {
   "name": "リザベンカプセル",
   "type": "naifuku",
   "form": "カプセル",
   "times": "",
   "aliases": [
    "リザベン",
    "トラニラスト",
    "りざべん"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "リボフラビン酪酸エステル錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "リボフラビン",
    "ハイボン",
    "りぼふらびん"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "レボフロキサシン錠",
   "type": "naifuku",
   "form": "錠剤",
   "times": "",
   "aliases": [
    "レボフロキサシン",
    "クラビット",
    "れぼふろきさしん"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ロラタジンDS",
   "type": "naifuku",
   "form": "こな薬",
   "times": "",
   "aliases": [
    "ロラタジンDS",
    "クラリチンDS"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "セファレキシン",
   "type": "naifuku",
   "form": "カプセル",
   "times": "",
   "aliases": [
    "セファレキシン",
    "ケフレックス",
    "せふぁれきしん"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ラリキシンドライシロップ",
   "type": "naifuku",
   "form": "こな薬",
   "times": "",
   "aliases": [
    "ラリキシン",
    "らりきしん"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "アモキシシリンカプセル250mg",
   "type": "naifuku",
   "form": "カプセル",
   "times": "",
   "aliases": [
    "アモキシシリン",
    "サワシリン",
    "あもきししりん"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "五苓散エキス顆粒",
   "fixedUsage": {"times": "3", "timing": ["朝", "昼", "夕"], "meal": "食前"},
   "type": "naifuku",
   "form": "こな薬",
   "times": "",
   "aliases": [
    "五苓散",
    "ゴレイサン"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "白虎加人参湯エキス顆粒",
   "type": "naifuku",
   "form": "こな薬",
   "times": "",
   "aliases": [
    "白虎加人参湯",
    "ビャッコカニンジントウ"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "アクトシン軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "アクトシン",
    "アクトシンo",
    "あくとしん"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "アクロマイシン軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "アクロマイシン",
    "アクロマイシンo",
    "あくろまいしん"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "アトラントクリーム",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "1",
   "aliases": [
    "アトラント",
    "アトラントcn",
    "あとらんと"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "アトラント外用液",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "1",
   "aliases": [
    "アトラント液",
    "アトラントlo"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "アポハイドローション",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "1",
   "aliases": [
    "アポハイド",
    "アポハイドlo",
    "あぽはいど"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "イオウ・カンフルローション",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "イオウカンフル",
    "イオウカンフルlo",
    "イオウ"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "エキザルベ",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "エキザルベ",
    "えきざるべ"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "エクラープラスター",
   "type": "gaiyou",
   "form": "貼り薬",
   "times": "1",
   "aliases": [
    "エクラー",
    "えくらー"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "エクロックゲル",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "1",
   "aliases": [
    "エクロック",
    "えくろっく"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "オロパタジン点眼液",
   "type": "gaiyou",
   "form": "点眼薬",
   "times": "4",
   "aliases": [
    "オロパタジン点眼",
    "パタノール",
    "オロパ点眼"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "クロベタゾールプロピオン酸エステルクリーム",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "クロ(P)cn",
    "クロPcn",
    "デルモベートクリーム",
    "デルモcn",
    "クロPcr",
    "クロ(P)cr"
   ],
   "note": "",
   "adopted": true,
   "common": true
  },
  {
   "name": "コムクロシャンプー",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "1",
   "aliases": [
    "コムクロ",
    "こむくろ"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "サレックス軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "サレックス",
    "サレックスo",
    "されっくす"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "スピール膏M",
   "type": "gaiyou",
   "form": "貼り薬",
   "times": "1",
   "aliases": [
    "スピール膏",
    "スピール",
    "すぴーる"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "スピラゾンクリーム",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "スピラゾンcn",
    "スピラゾンクリーム",
    "スピラゾンcr"
   ],
   "note": "",
   "adopted": true,
   "common": true
  },
  {
   "name": "スミスリンローション",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "1",
   "aliases": [
    "スミスリン",
    "すみすりん"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ゼフナートクリーム",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "1",
   "aliases": [
    "ゼフナート",
    "ぜふなーと"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "デキサメタゾン口腔用軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "デキサメタゾン口腔",
    "口腔用軟膏",
    "デキサ口腔",
    "アフタゾロン"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ドボベットゲル",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "1",
   "aliases": [
    "ドボベットゲル",
    "ドボベットg"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ドボベットフォーム",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "1",
   "aliases": [
    "ドボベットフォーム"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ニゾラールクリーム",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "1",
   "aliases": [
    "ニゾラール",
    "ニゾラールcn",
    "にぞらーる"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ニゾラールローション",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "1",
   "aliases": [
    "ニゾラールlo",
    "ニゾラールローション"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ネオメドロールEE軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "ネオメドロール",
    "ねおめどろーる",
    "NMEEo",
    "NMEE",
    "NMEEO"
   ],
   "note": "",
   "adopted": true,
   "common": true
  },
  {
   "name": "バラマイシン軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "バラマイシン",
    "ばらまいしん"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ビダラビンクリーム",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "ビダラビン",
    "アラセナ",
    "びだらびん"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "フルメタ軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "フルメタ",
    "フルメタo",
    "ふるめた"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "フルチカゾン点鼻液",
   "type": "gaiyou",
   "form": "点鼻薬",
   "times": "2",
   "aliases": [
    "フルチカゾン",
    "フルチカゾン点鼻",
    "ふるちかぞん"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ブロメライン軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "ブロメライン",
    "ぶろめらいん"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "プロトピック軟膏小児用",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "プロトピック小児",
    "プロトピック小児用"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ベピオローション",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "1",
   "aliases": [
    "ベピオlo",
    "ベピオローション"
   ],
   "note": "",
   "adopted": true,
   "site": "顔"
  },
  {
   "name": "ボアラ軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "ボアラ",
    "ボアラo",
    "ぼあら"
   ],
   "note": "",
   "adopted": true,
   "common": true
  },
  {
   "name": "ボチシート",
   "type": "gaiyou",
   "form": "貼り薬",
   "times": "1",
   "aliases": [
    "ボチシート",
    "ぼちしーと"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ユベラ軟膏",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "ユベラo",
    "ユベラ軟膏"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ラピフォートワイプ",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "1",
   "aliases": [
    "ラピフォート",
    "らぴふぉーと"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "リンデロンVGクリーム",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "VGクリーム",
    "リンデロンVGcn"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "リンデロンVGローション",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "VGローション",
    "リンデロンVGlo"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ロキソプロフェンNaテープ",
   "type": "gaiyou",
   "form": "貼り薬",
   "times": "1",
   "aliases": [
    "ロキソニンテープ",
    "ロキソプロフェンテープ",
    "ロキソテープ"
   ],
   "note": "",
   "adopted": true
  },
  {
   "name": "ロコイドクリーム",
   "type": "gaiyou",
   "form": "ぬり薬",
   "times": "2",
   "aliases": [
    "ロコイドcn",
    "ロコイドクリーム",
    "ロコイドcr"
   ],
   "note": "",
   "adopted": true,
   "common": true
  }
 ],
 "sets": [
  {
   "name": "しみ3つ",
   "aliases": [
    "しみ3つ",
    "シミ3つ",
    "しみ3",
    "シミ3ツ",
    "しみ三つ",
    "シメ3つ",
    "しみ37",
    "シミ37",
    "シメ37",
    "しみ3ツ"
   ],
   "lines": [
    "シナール配合錠 3T",
    "トラネキサム酸錠 3T",
    "ユベラ錠50mg 3T"
   ],
   "usage": "3×N"
  }
 ],
 "sites": [
  {
   "label": "顔保湿",
   "aliases": [
    "カオホシツ",
    "かおほしつ",
    "顔ほしつ",
    "顔ホシツ"
   ],
   "common": true
  },
  {
   "label": "顔ニキビ",
   "aliases": [
    "カオニキビ",
    "かおにきび",
    "顔にきび"
   ],
   "common": true
  },
  {
   "label": "全身保湿",
   "aliases": [
    "ゼンシンホシツ",
    "全身ホシツ",
    "ぜんしんほしつ"
   ],
   "common": true
  },
  {
   "label": "手保湿",
   "aliases": [
    "テホシツ",
    "手ホシツ",
    "てほしつ"
   ],
   "common": true
  },
  {
   "label": "虫さされ",
   "aliases": [
    "ムシササレ",
    "虫ササレ",
    "むしさされ"
   ],
   "common": true
  },
  {
   "label": "顔・体ニキビ",
   "aliases": [
    "カオカラダニキビ",
    "カオ・カラダニキビ",
    "顔・からだニキビ",
    "顔体ニキビ"
   ],
   "common": true
  },
  {
   "label": "下肢",
   "aliases": [
    "カシ",
    "かし"
   ],
   "common": true
  },
  {
   "label": "体わるいところ",
   "aliases": [
    "カラダワルイトコロ",
    "体ワルイトコロ",
    "からだわるいところ"
   ],
   "common": true
  },
  {
   "label": "顔わるいところ",
   "aliases": [
    "カオワルイトコロ",
    "かおわるいところ"
   ],
   "common": true
  },
  {
   "label": "顔かゆいところ",
   "aliases": [
    "カオカユイトコロ",
    "かおかゆいところ"
   ],
   "common": true
  },
  {
   "label": "顔・首わるいところ",
   "aliases": [
    "カオクビワルイトコロ",
    "カオ・クビワルイトコロ",
    "顔首わるいところ"
   ],
   "common": true
  },
  {
   "label": "体かゆいところ",
   "aliases": [
    "カラダカユイトコロ",
    "体カユイトコロ",
    "からだかゆいところ"
   ],
   "common": true
  },
  {
   "label": "からだ保湿",
   "aliases": [
    "カラダホシツ",
    "からだほしつ",
    "体保湿"
   ]
  },
  {
   "label": "足のつめ",
   "aliases": [
    "アシノツメ",
    "あしのつめ",
    "足爪",
    "足の爪"
   ]
  },
  {
   "label": "手のつめ",
   "aliases": [
    "テノツメ",
    "てのつめ",
    "手爪"
   ]
  },
  {
   "label": "ゆび",
   "aliases": [
    "ユビ",
    "指"
   ]
  },
  {
   "label": "足かかと",
   "aliases": [
    "アシカカト",
    "あしかかと",
    "かかと"
   ]
  },
  {
   "label": "足のうら",
   "aliases": [
    "アシノウラ",
    "足裏"
   ]
  },
  {
   "label": "くび",
   "aliases": [
    "クビ",
    "首"
   ]
  },
  {
   "label": "わき",
   "aliases": [
    "ワキ",
    "腋"
   ]
  },
  {
   "label": "手",
   "aliases": [
    "テ"
   ]
  },
  {
   "label": "足",
   "aliases": [
    "アシ"
   ]
  },
  {
   "label": "くちわるいところ",
   "aliases": [
    "クチワルイトコロ",
    "口わるいところ"
   ]
  },
  {
   "label": "足かさかさ",
   "aliases": [
    "アシカサカサ",
    "足カサカサ",
    "あしかさかさ"
   ]
  },
  {
   "label": "足汗止め",
   "aliases": [
    "アシアセドメ",
    "足アセドメ",
    "足汗どめ",
    "あしあせどめ"
   ]
  },
  {
   "label": "オヤユビ",
   "aliases": [
    "おやゆび",
    "親指"
   ]
  },
  {
   "label": "からだ",
   "aliases": [
    "カラダ",
    "体"
   ]
  },
  {
   "label": "頭",
   "aliases": [
    "アタマ",
    "あたま"
   ]
  },
  {
   "label": "顔",
   "aliases": [
    "かお",
    "カオ"
   ],
   "common": true
  },
  {
   "label": "キズ",
   "aliases": [
    "きず",
    "傷"
   ],
   "common": true
  },
  {
   "label": "目のまわり",
   "aliases": [
    "メノマワリ",
    "めのまわり",
    "目の周り",
    "目まわり"
   ]
  },
  {
   "label": "手足",
   "aliases": [
    "テアシ",
    "てあし"
   ]
  },
  {
   "label": "体・頭",
   "aliases": [
    "カラダアタマ",
    "体アタマ",
    "体頭",
    "からだあたま"
   ]
  },
  {
   "label": "鼻・口わるいところ",
   "aliases": [
    "ハナクチワルイトコロ",
    "ハナ・クチワルイトコロ",
    "鼻口わるいところ",
    "はなくちわるいところ"
   ]
  },
  {
   "label": "手わるいところ",
   "aliases": [
    "テワルイトコロ",
    "手ワルイトコロ",
    "てわるいところ"
   ]
  },
  {
   "label": "首・手",
   "aliases": [
    "クビテ",
    "首手",
    "くびて"
   ]
  },
  {
   "label": "顔よいところ",
   "aliases": [
    "カオヨイトコロ",
    "顔ヨイトコロ",
    "顔良いところ",
    "顔よい時",
    "かおよいところ",
    "カオヨイトキ"
   ]
  },
  {
   "label": "くちよいとき",
   "aliases": [
    "クチヨイトキ",
    "口よいとき",
    "口良い時",
    "クチヨイトコロ"
   ]
  },
  {
   "label": "くちまわり",
   "aliases": [
    "クチマワリ",
    "口まわり",
    "口のまわり",
    "くちのまわり"
   ]
  },
  {
   "label": "爪",
   "aliases": [
    "つめ",
    "ツメ"
   ]
  },
  {
   "label": "うで",
   "aliases": [
    "ウデ",
    "腕"
   ]
  },
  {
   "label": "体アセモ",
   "aliases": [
    "カラダアセモ",
    "体あせも",
    "からだあせも",
    "体アセモ"
   ]
  },
  {
   "label": "顔赤いところ",
   "aliases": [
    "カオアカイトコロ",
    "顔あかいところ",
    "かおあかいところ",
    "カオ赤いところ"
   ]
  },
  {
   "label": "体アトピー",
   "aliases": [
    "カラダアトピー",
    "からだアトピー",
    "体あとぴー"
   ]
  },
  {
   "label": "顔・首",
   "aliases": [
    "カオクビ",
    "顔首",
    "かおくび",
    "カオ・クビ"
   ]
  },
  {
   "label": "手の親指",
   "aliases": [
    "テノオヤユビ",
    "手のおやゆび",
    "手のオヤユビ",
    "てのおやゆび"
   ]
  }
 ],
 "layout": {
  "_説明": "座標は薬袋の横幅を1000、縦を1414とした相対値です（左上が0,0）。実寸(mm)は sizes で指定し、印字位置のズレは calibration(mm) で補正します。",
  "sizes": {
   "small": {
    "label": "小",
    "width_mm": 100,
    "height_mm": 140
   },
   "A5": {
    "label": "大",
    "width_mm": 148,
    "height_mm": 210
   }
  },
  "calibration": {
   "naifuku": {
    "small": {
     "dx": 0,
     "dy": 0
    },
    "A5": {
     "dx": 0,
     "dy": 0
    }
   },
   "gaiyou": {
    "small": {
     "dx": 0,
     "dy": 0
    },
    "A5": {
     "dx": 0,
     "dy": 0
    }
   }
  },
  "templates": {
   "naifuku": {
    "label": "のみぐすり",
    "fields": {
     "name": {
      "x": 812,
      "y": 305,
      "align": "right",
      "size": 52,
      "maxw": 430
     },
     "year": {
      "x": 572,
      "y": 512,
      "align": "right",
      "size": 34
     },
     "month": {
      "x": 664,
      "y": 512,
      "align": "center",
      "size": 34
     },
     "day": {
      "x": 806,
      "y": 512,
      "align": "center",
      "size": 34
     },
     "times": {
      "x": 375,
      "y": 607,
      "align": "center",
      "size": 62,
      "bold": true,
      "maxw": 120
     },
     "days": {
      "x": 645,
      "y": 607,
      "align": "center",
      "size": 62,
      "bold": true,
      "maxw": 170
     },
     "powder": {
      "x": 722,
      "y": 708,
      "align": "center",
      "size": 42,
      "bold": true,
      "maxw": 120
     },
     "capsule": {
      "x": 722,
      "y": 762,
      "align": "center",
      "size": 42,
      "bold": true,
      "maxw": 120
     },
     "tablet": {
      "x": 722,
      "y": 815,
      "align": "center",
      "size": 42,
      "bold": true,
      "maxw": 120
     },
     "dose_note": {
      "x": 846,
      "align": "left",
      "size": 30,
      "maxw": 148,
      "_メモ": "縦位置は最後に記入した1回量の行に合わせる"
     },
     "interval": {
      "x": 615,
      "y": 876,
      "align": "center",
      "size": 38,
      "maxw": 95
     },
     "tonpuku_amount": {
      "x": 395,
      "y": 1040,
      "align": "center",
      "size": 36,
      "maxw": 90
     },
     "tonpuku_count": {
      "x": 578,
      "y": 1040,
      "align": "center",
      "size": 36,
      "maxw": 90
     }
    },
    "marks": {
     "朝": {
      "x": 232,
      "y": 880,
      "rx": 32,
      "ry": 29
     },
     "昼": {
      "x": 298,
      "y": 880,
      "rx": 32,
      "ry": 29
     },
     "夕": {
      "x": 377,
      "y": 880,
      "rx": 32,
      "ry": 29
     },
     "ねる前": {
      "x": 477,
      "y": 880,
      "rx": 60,
      "ry": 29
     },
     "食後": {
      "x": 352,
      "y": 935,
      "rx": 48,
      "ry": 29
     },
     "食前": {
      "x": 492,
      "y": 935,
      "rx": 48,
      "ry": 29
     },
     "食間": {
      "x": 633,
      "y": 935,
      "rx": 48,
      "ry": 29
     },
     "痛い時": {
      "x": 345,
      "y": 1085,
      "rx": 52,
      "ry": 25
     },
     "発熱時": {
      "x": 478,
      "y": 1085,
      "rx": 52,
      "ry": 25
     },
     "かゆい時": {
      "x": 628,
      "y": 1085,
      "rx": 64,
      "ry": 25
     }
    }
   },
   "gaiyou": {
    "label": "外用薬",
    "fields": {
     "name": {
      "x": 818,
      "y": 305,
      "align": "right",
      "size": 52,
      "maxw": 430
     },
     "year": {
      "x": 568,
      "y": 552,
      "align": "right",
      "size": 34
     },
     "month": {
      "x": 660,
      "y": 552,
      "align": "center",
      "size": 34
     },
     "day": {
      "x": 806,
      "y": 552,
      "align": "center",
      "size": 34
     },
     "times": {
      "x": 525,
      "y": 642,
      "align": "center",
      "size": 62,
      "bold": true,
      "maxw": 120
     },
     "site": {
      "x": 722,
      "y": 646,
      "align": "left",
      "size": 46,
      "bold": true,
      "maxw": 255
     },
     "zayaku_temp": {
      "x": 550,
      "y": 882,
      "align": "center",
      "size": 36,
      "maxw": 90
     }
    },
    "marks": {
     "ぬり薬": {
      "x": 252,
      "y": 722,
      "rx": 72,
      "ry": 29
     },
     "点眼薬": {
      "x": 500,
      "y": 722,
      "rx": 72,
      "ry": 29
     },
     "点鼻薬": {
      "x": 745,
      "y": 722,
      "rx": 72,
      "ry": 29
     },
     "点耳薬": {
      "x": 250,
      "y": 778,
      "rx": 72,
      "ry": 29
     },
     "貼り薬": {
      "x": 500,
      "y": 778,
      "rx": 72,
      "ry": 29
     },
     "うがい薬": {
      "x": 745,
      "y": 778,
      "rx": 78,
      "ry": 29
     },
     "トローチ": {
      "x": 250,
      "y": 832,
      "rx": 78,
      "ry": 29
     },
     "消毒用の薬": {
      "x": 500,
      "y": 832,
      "rx": 80,
      "ry": 29
     },
     "坐薬": {
      "x": 245,
      "y": 885,
      "rx": 72,
      "ry": 29
     }
    }
   }
  }
 },
 "rules": [
  {
   "id": "r1",
   "kind": "all_of",
   "words": [
    "シナール",
    "ユベラ",
    "トラネキサム"
   ],
   "size": "A5",
   "memo": "しみの3点処方"
  },
  {
   "id": "r2",
   "kind": "containers_at_least",
   "n": 2,
   "size": "A5",
   "memo": "混合容器（2番=30g・3番=50g）が2個以上（例: サヘパ-3×2）"
  },
  {
   "id": "r3",
   "kind": "qty_at_least",
   "unit": "本",
   "n": 9,
   "size": "A5",
   "memo": "軟膏などの本数が9本以上"
  },
  {
   "id": "r4",
   "kind": "any_of",
   "words": [
    "ヘパリン類似物質",
    "ヒルドイド"
   ],
   "size": "A5",
   "memo": "ヘパリン系の薬（ヘパlo・ヘパcn など。ロヘパなどの混合軟膏は除く）"
  },
  {
   "id": "r5",
   "kind": "containers_at_least",
   "n": 1,
   "minNo": 4,
   "size": "A5",
   "memo": "4番の容器（100g）は1個でも大きい袋（例: サヘパ-4）"
  }
 ],
 "presets": [
  {
   "title": "しみ3点セット",
   "text": "しみ3つ\n3×N 60TD"
  }
 ],
 "renamed": {
  "ゲンタシン軟膏": "ゲンタマイシン硫酸塩軟膏",
  "デルモベート軟膏": "クロベタゾールプロピオン酸エステル軟膏",
  "ルリコンクリーム": "ルリコナゾールクリーム",
  "ラミシールクリーム": "テルビナフィン塩酸塩クリーム",
  "アクアチムクリーム": "ナジフロキサシンクリーム",
  "アセトアミノフェン錠": "カロナール錠200",
  "プレドニゾロン錠": "プレドニン錠5mg",
  "セフカペンピボキシル塩酸塩錠": "フロモックス錠",
  "キンダベート軟膏": "クロベタゾン酪酸エステル軟膏",
  "ダラシンTゲル": "クリンダマイシンゲル",
  "ヘパリン類似物質クリーム": "ヘパリン類似物質油性クリーム"
 },
 "siteRenamed": {
  "かお": "顔",
  "おやゆび": "オヤユビ"
 },
 "handBase": {
  "conf": {
   "A": {
    "3": {
     "3": 2
    },
    "7": {
     "7": 1
    },
    "l": {
     "l": 5
    },
    "パ": {
     "1": 2,
     "n": 2,
     "バ": 2,
     "o": 2,
     "パ": 1,
     "ハ": 2
    },
    "湿": {},
    "保": {},
    "o": {
     "0": 4,
     "o": 7
    },
    "M": {
     "m": 2,
     "M": 1,
     "n": 2
    },
    "G": {
     "G": 4
    },
    "ビ": {
     "T": 2
    },
    "ユ": {},
    "ル": {},
    "ゲ": {
     "T": 2,
     "-": 2
    },
    "ー": {
     "4": 2,
     "-": 6
    },
    "リ": {},
    "ク": {
     "7": 2
    },
    "ミ": {},
    "シ": {
     "2": 2,
     "シ": 2
    },
    "ン": {
     "ン": 1
    },
    "ゾ": {},
    "ラ": {
     "(": 2,
     "ラ": 1
    },
    "ピ": {
     "r": 2
    },
    "オ": {
     "オ": 3
    },
    "ム": {},
    "プ": {
     "0": 2
    },
    "ト": {
     "4": 2,
     "7": 2
    },
    "レ": {},
    "サ": {
     "サ": 1
    },
    "刺": {},
    "虫": {},
    "イ": {
     "イ": 1
    },
    "マ": {},
    "ノ": {},
    "ヘ": {
     "ー": 3
    },
    "ロ": {
     "3": 2
    },
    "コ": {
     "2": 2
    },
    "ツ": {},
    "r": {},
    "c": {
     "c": 1
    },
    "ス": {
     "ス": 1
    },
    "ニ": {
     "ニ": 1
    },
    "顔": {},
    "ュ": {},
    "ウ": {
     "ウ": 1
    },
    "ジ": {},
    "タ": {},
    "ダ": {},
    "カ": {},
    "ズ": {},
    "N": {
     "N": 1
    },
    "周": {},
    "頭": {},
    "体": {
     "体": 1
    },
    "キ": {},
    "所": {},
    "n": {
     "n": 1
    },
    "ア": {},
    "ボ": {
     "ボ": 1
    },
    "手": {},
    "首": {
     "首": 1
    },
    "悪": {},
    "指": {},
    "ベ": {},
    "身": {
     "身": 1
    },
    "ホ": {},
    "ナ": {
     "ナ": 1
    }
   },
   "J": {
    "3": {
     "3": 1
    },
    "7": {},
    "パ": {
     "パ": 9,
     "バ": 2
    },
    "ヘ": {
     "ヘ": 7,
     "ハ": 3
    },
    "湿": {},
    "保": {},
    "o": {
     "0": 3,
     "ン": 2,
     "o": 1,
     "ル": 2
    },
    "M": {},
    "G": {},
    "ビ": {
     "ビ": 2,
     "ン": 2
    },
    "ユ": {},
    "ヤ": {},
    "オ": {
     "オ": 5
    },
    "ル": {
     "ル": 4
    },
    "ゲ": {
     "ク": 2
    },
    "ー": {
     "ー": 4,
     "-": 2
    },
    "リ": {
     "リ": 3
    },
    "ク": {
     "ク": 3
    },
    "キ": {
     "ヤ": 2
    },
    "ニ": {},
    "カ": {
     "カ": 1
    },
    "ミ": {},
    "ン": {
     "ン": 3
    },
    "ゾ": {},
    "ピ": {
     "ピ": 1
    },
    "ス": {
     "ス": 2
    },
    "ツ": {
     "ク": 2
    },
    "シ": {
     "シ": 3,
     "ッ": 2
    },
    "ホ": {
     "ド": 2
    },
    "ム": {
     "ム": 1
    },
    "プ": {},
    "ト": {
     "ト": 2,
     "コ": 2
    },
    "レ": {},
    "サ": {
     "サ": 3
    },
    "刺": {},
    "虫": {},
    "イ": {
     "イ": 1,
     "ソ": 2
    },
    "マ": {},
    "ノ": {},
    "ロ": {
     "ロ": 5
    },
    "コ": {
     "ト": 2
    },
    "悪": {
     "カ": 2
    },
    "顔": {},
    "体": {},
    "l": {},
    "r": {},
    "ア": {
     "ア": 1
    },
    "ゼ": {
     "ゼ": 1
    },
    "ュ": {},
    "ッ": {
     "ッ": 1
    },
    "ォ": {},
    "ウ": {
     "ウ": 1
    },
    "ベ": {},
    "ジ": {
     "ジ": 1
    },
    "タ": {},
    "ラ": {
     "ラ": 1
    },
    "ダ": {},
    "ズ": {},
    "E": {
     "l": 2
    },
    "N": {
     "N": 1
    },
    "周": {},
    "頭": {},
    "所": {},
    "首": {
     "ロ": 2
    },
    "n": {
     "n": 1
    },
    "c": {
     "c": 1
    },
    "ボ": {},
    "手": {},
    "フ": {},
    "ナ": {
     "ナ": 1
    },
    "全": {
     "全": 1
    }
   },
   "B": {
    "3": {
     "3": 2
    },
    "7": {
     "7": 2
    },
    "o": {
     "0": 3,
     "o": 6
    },
    "M": {
     "M": 2
    },
    "G": {
     "G": 3
    },
    "ユ": {
     "ユ": 1
    },
    "ヤ": {
     "ヤ": 1
    },
    "オ": {
     "オ": 7
    },
    "ル": {
     "ル": 2
    },
    "ー": {
     "ー": 5,
     "-": 2
    },
    "リ": {
     "リ": 3
    },
    "ク": {
     "ク": 3
    },
    "ビ": {
     "ビ": 2
    },
    "キ": {
     "キ": 3
    },
    "カ": {
     "カ": 5
    },
    "ミ": {
     "ミ": 2
    },
    "シ": {
     "シ": 9
    },
    "ス": {
     "ス": 2
    },
    "パ": {
     "パ": 8,
     "バ": 3
    },
    "ヘ": {
     "ヘ": 8
    },
    "ツ": {
     "ッ": 3
    },
    "ト": {
     "ト": 3
    },
    "ン": {
     "ン": 3
    },
    "イ": {
     "イ": 3
    },
    "ロ": {
     "ロ": 4
    },
    "コ": {
     "コ": 2
    },
    "l": {
     "ル": 2
    },
    "ホ": {
     "ホ": 1,
     "ネ": 2
    },
    "r": {
     "r": 1
    },
    "c": {
     "c": 2
    },
    "ッ": {
     "ッ": 2
    },
    "ア": {
     "ア": 2
    },
    "ゼ": {
     "ゼ": 1
    },
    "ニ": {
     "ニ": 1
    },
    "ュ": {
     "ュ": 1
    },
    "ォ": {
     "ォ": 1
    },
    "ウ": {
     "ウ": 1
    },
    "ピ": {},
    "ベ": {
     "ベ": 2
    },
    "ジ": {
     "ジ": 1
    },
    "タ": {},
    "ラ": {
     "ラ": 2
    },
    "E": {
     "l": 2
    },
    "N": {
     "N": 1
    },
    "ノ": {
     "ノ": 1
    },
    "体": {
     "体": 1
    },
    "ゲ": {},
    "n": {
     "n": 1
    },
    "ボ": {},
    "手": {},
    "ゾ": {},
    "サ": {
     "サ": 2
    },
    "フ": {},
    "ナ": {
     "ナ": 2
    },
    "身": {
     "身": 1
    },
    "全": {
     "全": 1
    }
   }
  },
  "n": 65
 }
};
